...
class per_partner_simulator():
    ...
    def _first_time_getting_days(self):
        ...
        for rows in date_in_day.itertuples(index=True):
            productsOfDay.add(getattr(rows, "product_id"))
            #self.productsSeenSoFar.add(getattr(rows, "product_id"))
        # getting the new excluded day
        #self.productsActuallyExcluded = productsOfDay.intersection(self.productsToExclude)
        # self.productsSeenSoFar=self.productsSeenSoFar.join(productsOfDay)
        self.go_to_optimizer(productsOfDay)
